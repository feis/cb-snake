...
int main() {
	...
	while (1) {
		...
		int fieldCellStates[WIDTH][HEIGHT];
		...
		} while (fieldCellStates[foodPositionX][foodPositionY] != EMPTY);
		...
		int fieldCellDirections[WIDTH][HEIGHT];
		...
		while (1) {
			...
			if (state == FOOD) {
        		...
        		// 將場地狀態中，蛇尾的位置修改為空地
				fieldCellStates[tailPositionX][tailPositionY] = EMPTY;
